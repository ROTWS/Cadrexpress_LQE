...
function buildGenerationContext() {
  ...
  // Ajouter la charte graphique personnalisée si elle existe
  let brandBlock = '';
  if (STATE.customBrand && STATE.customBrand.trim()) {
    brandBlock = '\n\n━━━ CHARTE GRAPHIQUE DE LA STRUCTURE (à appliquer en PRIORITÉ pour la maquette) ━━━\n' + STATE.customBrand + '\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
  }

  return `CONTEXTE DU CADRAGE :\n${answersBlock}${brandBlock}\n\n${
...
// ============================================
// FIX CRITIQUE : ouvrir la maquette via Blob URL
// pour que le JavaScript s'exécute correctement
// ============================================
function openMaquette() {
  const content = STATE.deliverables.maquette;
  if (!content) return;

  // Créer un Blob HTML et l'ouvrir via URL — garantit l'exécution du JS
  const blob = new Blob([content], { type: 'text/html;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  window.open(url, '_blank');

  // Libérer après 60 s (le temps que la page charge)
...
<style>
  ...
  th, td { border: 1px solid #d9d9d9; padding: 0.5rem 0.8rem; text-align: left; vertical-align: top; }
...
function markdownToHtml(md) {
  ...
  html = html.replace(/^\|(.+)\|\r?\n\|[\s\-:|]+\|\r?\n((?:\|.+\|\r?\n?)+)/gm, (m, header, rows) => {
    ...
    const trs = rows.trim().split(/\r?\n/).map(row => {
      ...
      // Enlever premier et dernier élément vides
