...
// ============================================
// Sélection de source — FIX : mise à jour fiable
// ============================================
function selectSource(type, el) {
  // FIX : forcer la mise à jour de l'état
  ...
  // Debug visible (aide le diagnostic)
...
async function handleAudio(file) {
  if (!file) return;
  audioFileData = file;
  // FIX : forcer currentSource sur audio dès qu'on dépose un fichier
...
// ============================================
// PDF — FIX : bascule automatique sur onglet PDF
// ============================================
async function handlePdf(file) {
  if (!file) return;
  pdfFileData = file;
  // FIX : forcer currentSource sur pdf dès qu'on dépose un fichier
  ...
  `;
  log(1, '📄 Fichier chargé : ' + file.name + ' — cliquez sur "Analyser maintenant"');
  console.log('PDF chargé, currentSource =', STATE.currentSource);
}
...
function toggleDictation() {
  ...
  // FIX : forcer currentSource sur dictation quand on démarre
...
// ============================================
// Analyse source — FIX : détection automatique de la source
// ============================================
async function analyzeSource() {
  ...
  // FIX : détection intelligente de la source si currentSource n'est pas fiable
  // Priorité : 1) fichier PDF chargé, 2) fichier audio, 3) dictée, 4) texte collé
  let actualSource = STATE.currentSource;

  // Re-vérifier en fonction du panneau actif
  ...
  try {
    ...
    if (nbFilled === 0) {
      ...
      log(1, '🎉 Regardez la colonne de droite pour voir les réponses !', 'success');
    }
  } catch (e) {
